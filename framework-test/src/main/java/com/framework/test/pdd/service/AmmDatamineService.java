...
import com.framework.common.utils.URIUtil;
...
import com.framework.test.pdd.entity.MultipleSkuEntity;
import com.framework.test.pdd.entity.SimpleSkuEntity;
...
public class AmmDatamineService extends LoginService implements IDatamineService {
	...
	public List<String> getGoodsList(String type, Integer page) throws Exception {
		...
		// 查询页码是否超过最大值
		if (page == null || page <= 0) {
			throw new ServiceException("页数【", maxPage, "】有误");
		}
		...
		if (StringUtils.isEmpty(urlStr)) {
			throw new ServiceException("商品爬取地址为空");
		}
	...
	private GoodsEntity getGoods(GoodsEntity goods) throws Exception {
		...
		if (StringUtils.isEmpty(name)) {
			throw new ServiceException("商品标题为空");
		}
		goods.setName(name);
		...
		if (StringUtils.isEmpty(currentprice)) {
			throw new ServiceException("商品原价为空");
		}
		...
		try {
			...
			if (simpleSkuIdList.size() == 1) {
				...
				throw new ServiceException("SKU信息超过2个商品抛弃");
			}

		} catch (Exception e) {
			throw new ServiceException(e, "爬取SKU信息失败");
		}
		...
		try {
			...
			throw new ServiceException(e, "爬取script信息失败");
		}
		...
		try {
			...
			throw new ServiceException(e, "爬取预览图片失败");
		}
		...
		if (StringUtils.isNotEmpty(scriptsInfoMap.get("propertyMemoMap"))) {
			...
			try {
				...
				throw new ServiceException(e, "爬取补充sku名称信息失败");
			}
		}
		...
		try {
			...
			if (StringUtils.isEmpty(sibInfo)) {
				throw new ServiceException("sibInfo为空");
			}
			sibJson = JSONObject.parseObject(sibInfo);
		} catch (Exception e) {
			throw new ServiceException(e, "爬取sku组合价格信息失败");
		} finally {
		...
		try {
			...
			throw new ServiceException(e, "组合sku原价，活动价，库存等信息失败");
		}
		...
		if (descImages.isEmpty()) {
			try {
				...
				throw new ServiceException(e, "爬取详情图片失败");
			}
		}
