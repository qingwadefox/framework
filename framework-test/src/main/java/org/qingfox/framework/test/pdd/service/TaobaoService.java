...
public class TaobaoService extends LoginService implements IBuyService {
     ...
     * 登录 {@inheritDoc}
     * 
     * @see org.qingfox.framework.test.pdd.service.inf.IBuyService#login(java.lang.String,
     *      java.lang.String)
     * @author Administrator 2018年1月5日 Administrator
     * @throws Exception
     */
    ...
    public void buy(OrderEntity order) throws Exception {
        ...
        synchronized (lockObject) {
            ...
            try {
                ...
                throw new RuntimeException("获取省映射ID失败", e);
            }
            logger.debug("buyProvinceId【", buyProvinceId, "】");
            if (StringUtils.isEmpty(buyProvinceId)) {
                throw new RuntimeException("获取buyProvinceId为空");
            }
            ...
            try {
                ...
                throw new RuntimeException("验证市ID映射失败", e);
            }
            logger.debug("buyCityId【", buyCityId, "】");
            if (StringUtils.isEmpty(buyCityId)) {
                throw new RuntimeException("获取buyCityId为空");
            }
            ...
            if (!WebDriverUtils.isForward(driver, URLConstant.TAOBAO_ITEM_BUY)) {
                throw new RuntimeException("打开购买页面失败");
            }
            ThreadUtil.sleep(1000);
            logger.info("选择SKU");
            ...
            if (!WebDriverUtils.isForward(driver, URLConstant.TAOBAO_BUY)) {
                throw new RuntimeException("跳转购买信息页面失败");
            }
            ...
            if (sellAmount < Float.parseFloat(realPay)) {
                throw new RuntimeException("卖单金额【" + sellAmount + "】大于买单金额【" + realPay + "】不对应");
            }
     ...
     * 获取订单列表 {@inheritDoc}
     * 
     * @see org.qingfox.framework.test.pdd.service.inf.IBuyService#getOrderList(java.util.List)
