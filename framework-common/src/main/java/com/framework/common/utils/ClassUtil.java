...
public class ClassUtil {
	...
	public static String getClassPath() throws UnsupportedEncodingException {
		return URLDecoder.decode(
				ClassUtil.class
						...
						.replace(
								ClassUtil.class.getPackage().getName()
										.replace(".", File.separator)
	...
	public static String getClassPath(String path) {
		return URLDecoder.decode(ClassUtil.class.getResource(path).getPath()
				.replace("/", File.separator));
	}
		...
		for (File file : fileList) {
			...
			if (file.getName().endsWith(".class")) {
				String classPath = file.getPath()
						.replace(dirPath + File.separator, "")
						.replace(File.separator, ".");
	...
	public static List<String> getClasses(JarFile jarFile) {
		...
		while (enumeration.hasMoreElements()) {
			...
			if (classPath.endsWith(".class")) {
				classPath = classPath.substring(0,
						classPath.lastIndexOf(".class")).replace("/", ".");
				classList.add(classPath);
			}
		}
		return classList;
